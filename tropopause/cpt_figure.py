...
def cpt_fig(data_sel, cpt_alt, cpt_temp, data_METOP):
    ...
    axs[0].set_title(f"latitude {data_sel.attrs['lat_limit']}, time {data_sel.time.values.astype('datetime64[M]')}")
    ...
    axs[1].set_title(f"latitude {data_METOP.attrs['lat_limit']}, time {data_METOP.attrs['time_limit']}")
